...
void Filter_b()
{
    ...
    int q, c1, c2, r1, r2, row = 0, col = 0, temp1 ;
    for(int n = 1; n <= 4; ++n)
    {
        cin >> q;
        if (q == 1)
...
void Filter_c()
{
    for ( int i = 0 ; i < SIZE ; i++)
    {
        for ( int j = 0 ; j < SIZE ; j++)
        {
            image2[i][j]=( image[i-1][j-1]+image[i-1][j]+image[i-1][j+1]+image[i][j-1]+image[i][j]+image[i][j+1]+image[i+1][j-1]+image[i+1][j]
                            +image[i+1][j+1])/ 12;
        }
    }
...
void Lighten()
{
    ...
    for (int i = 0; i < SIZE; i++)
        for (int j = 0; j< SIZE; j++)
            for(int k = 0; k < 3; ++k) {
                image_2[i][j][k] = (image_1[i][j][k])*1.5;
                if (image_2[i][j][k]> 255)
                {
                    image_2[i][j][k] = 0;
                }
            }
